...
$breadcrumbs = [
    ...
    ['label' => 'Authentication', 'icon' => '<svg class="w-4 h-4 text-indigo-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"/></svg>']
];
@endphp
...
<x-admin-layout title="Authentication Audit Logs" :breadcrumbs="$breadcrumbs">
    ...
    <script>
        ...
        function showMessage(message, type = 'success') {
            ...
            container.innerHTML = `
                <div class="${bgColor} border-l-4 rounded-lg p-4 mb-4">
        ...
        function renderUserActivity(data) {
            ...
            if (!can_manage) {
                ...
                if (user.account_status === 'active') {
                    html += `
                        <button onclick="showSuspendForm()" class="flex items-center justify-center gap-2 px-4 py-3 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button onclick="showLockForm()" class="flex items-center justify-center gap-2 px-4 py-3 bg-orange-600 hover:bg-orange-700 text-white font-semibold rounded-lg transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                            </svg>
                            Lock
                    ...
                    html += `
                        <button onclick="unlockUser()" class="flex items-center justify-center gap-2 px-4 py-3 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                html += `
                        <button onclick="showResetPasswordForm()" class="flex items-center justify-center gap-2 px-4 py-3 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-lg transition">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z"></path>
                            </svg>
                            Reset Password
        ...
        function executePasswordReset() {
            ...
            .then(data => {
                ...
                if (data.success) {
                    // Show success message (keep form visible so message can be seen)
                    showMessage(data.message, 'success');
                    // Hide the confirmation step to show message clearly
                    document.getElementById('passwordConfirmationStep').classList.add('hidden');
                    // Reload to show updated audit log
                    setTimeout(() => {
                        location.reload();
                    }, 2000);
                } else {
