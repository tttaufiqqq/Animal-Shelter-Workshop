...
<div id="medicalModal" class="hidden fixed inset-0 bg-black bg-opacity-50 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
    <div class="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <form method="POST" action="{{ route('medical-records.store') }}" class="p-6 space-y-4" id="medicalRecordForm" onsubmit="handleMedicalSubmit(event)">
            @csrf
            ...
            <div class="flex justify-end gap-3 pt-4">
                ...
                <button type="submit" id="medicalSubmitBtn" class="px-6 py-3 bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold rounded-lg hover:from-blue-600 hover:to-blue-700 transition duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                    <span id="medicalBtnText">
                        <i class="fas fa-plus mr-2"></i>Add Record
                    </span>
                    <span id="medicalBtnLoading" class="hidden">
...
function handleMedicalSubmit(event) {
    ...
    const btnText = document.getElementById('medicalBtnText');
    const btnLoading = document.getElementById('medicalBtnLoading');
