...
<div class="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
    ...
    <a href="{{ route('bookings.index-admin') }}"
       class="bg-white border-2 rounded-lg p-6 text-center {{ !request('status') ? 'border-purple-600 bg-purple-50' : 'border-gray-200' }}">
    ...
    <a href="{{ route('bookings.index-admin', ['status' => 'Pending']) }}"
       class="bg-white border-2 rounded-lg p-6 text-center {{ request('status') == 'Pending' ? 'border-yellow-600 bg-yellow-50' : 'border-gray-200' }}">
    ...
    <a href="{{ route('bookings.index-admin', ['status' => 'Confirmed']) }}"
       class="bg-white border-2 rounded-lg p-6 text-center {{ request('status') == 'Confirmed' ? 'border-blue-600 bg-blue-50' : 'border-gray-200' }}">
    ...
    <a href="{{ route('bookings.index-admin', ['status' => 'Completed']) }}"
       class="bg-white border-2 rounded-lg p-6 text-center {{ request('status') == 'Completed' ? 'border-green-600 bg-green-50' : 'border-gray-200' }}">
    ...
    <a href="{{ route('bookings.index-admin', ['status' => 'Cancelled']) }}"
       class="bg-white border-2 rounded-lg p-6 text-center {{ request('status') == 'Cancelled' ? 'border-red-600 bg-red-50' : 'border-gray-200' }}">
    ...
    <form method="GET" action="{{ route('bookings.index-admin') }}" class="space-y-4">
        ...
        <div class="flex items-center gap-2 mb-4">
            ...
            @if(request()->hasAny(['user_search', 'booking_id', 'date_from', 'date_to']))
                <span class="ml-auto px-3 py-1 bg-purple-100 text-purple-700 rounded-full text-xs font-medium">
        ...
        <div class="flex gap-3 pt-2">
            <button type="submit"
                    class="px-6 py-2 bg-purple-600 text-white rounded-lg font-semibold flex items-center gap-2">
                <i class="fas fa-search"></i>
                Search
            </button>
            <a href="{{ route('bookings.index-admin') }}"
               class="px-6 py-2 bg-gray-200 text-gray-700 rounded-lg font-semibold flex items-center gap-2">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <button type="button"
                                    ...
                                    class="inline-flex items-center gap-1 bg-purple-600 text-white px-3 py-1.5 rounded-lg text-xs font-semibold">
                        ...
                        <div class="bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[90vh] overflow-y-auto">
                            ...
                            <div class="bg-purple-600 border-b border-purple-700 p-6 text-white">
                                <div class="flex items-center justify-between">
                                    ...
                                    <button onclick="closeModal('bookingModal-{{ $booking->id }}')" class="text-white">
                            ...
                            <div class="p-6 space-y-6">
                                ...
                                @if($booking->adoptions->isNotEmpty())
                                    <button onclick="openAdoptionModal({{ $booking->id }})"
                                        ...
                                        <i class="fas fa-heart mr-2"></i>View Adoption Records
                                    </button>
                                @endif
                            </div>
                        </div>
                            ...
                            <div class="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-y-auto">
                                ...
                                <div class="bg-green-600 border-b border-green-700 p-6 text-white">
                                    <div class="flex items-center justify-between">
                                        ...
                                        <button onclick="closeAdoptionModal({{ $booking->id }})" class="text-white">
                                            <i class="fas fa-times text-xl"></i>
                                        </button>
                                    </div>
                                </div>
